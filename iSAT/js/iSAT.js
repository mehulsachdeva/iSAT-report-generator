!function(){
	...
	function drawPart(data, id, p,s){
		...
		if((p || (s==0)) && id!='Last'){			
			...
			.text(function(d,i){return remapp(data.keys[p][i],unikc[p+s]); })
	...
	function drawEdges(data, id,s){
		...
		var text;
		if($(this).parent().parent().prop("id")=="SalesAttempts")
			text= d.value +" moved from strata \"<b>"+remapp(d.key1,unikc[0])+"\"</b> to strata <b>\""+remapp(d.key2,unikc[1])+"\"</b>"
		else
			text= d.value +" moved from strata \"<b>"+remapp(d.key1,unikc[1])+"\"</b> to strata <b>\""+remapp(d.key2,unikc[2])+"\"</b>"
...
bP.selectSegment = function(data, m, s,s1){
	...
	bar.append("text")
		...
		.attr("y",barHeight/2)
		.attr("dy", ".35em")
    	.text(function(d,i) { return remapp((transpose(initial))[0][i],unikc[0]); });
	...
	function switchF(data,svg,visData){
		...
		data.forEach(function(biP,s){
			...
			bar.append("text")
				...
				.attr("y",barHeight/2)
				.attr("dy", ".35em")
    			.text(function(d,i) { return remapp((transpose(initial))[2][i],unikc[s]); });
			...
			bar.append("text")
				...
				.attr("y",barHeight/2)
				.attr("dy", ".35em")
    			.text(function(d,i) { return remapp((transpose(initial))[0][i],unikc[s+1]); });
		...
		data.forEach(function(biP,s){
			...
			for(i=0;i<nArray.length;i++){
				if(nArray[i][0]== nArray[i][2] && nArray[i][0] != nArray[i][1]) {
					console.log(remapp(nArray[i][0],unikc[0])+ "->" + remapp(nArray[i][1],unikc[1]) + "->" + remapp(nArray[i][2],unikc[2]) + " count = "+ nArray[i].deepSortAlpha);
				}
			}
	}
