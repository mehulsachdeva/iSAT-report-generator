  ...
  var html="";
    ...
    for(i=0;i<dataunique.length;i++){
      for(j=0;j<dataunique[i].length;j++){
        strata_list.push(dataunique[i][j]);
      }
    }
    ...
    console.log(allPossibleCases(alpha));
  	for(i=0;i<allPossibleCases(alpha).length;i++){
      console.log(allPossibleCases(alpha)[i].split("#"));
    ...
    for(i=1;i<list1.length;i++){
      ...
      for(j=0;j<list1[i].length;j++){
        ...
        //console.log(dataunique[j][list1[i][j]]);
      }
    }
    console.log(list3);
    for(i=0;i<list1.length;i++){
      list_data.push(list3.slice(0,cou-1));
      console.log(list3.slice(0,cou-1));
    ...
    */
    ...
    for(i=1;i<filtered.length;i++){
      ...
      for(j=0;j<filtered[i].length;j++){
        ...
        for(k=0;k<filtered[i][j][0].length;k++){
          if(filtered[i][j][0][k]!="0"){
            matchstr2.push(filtered[i][j][0][k]);
          }
        }
        matchstr1.push(matchstr2);
      }
      matchstr.push(matchstr1);
    }
    matchstr.unshift("");
    ...
    for(i=0;i<filtered.length;i++){
      for(j=0;j<filtered[i].length;j++){
        filtered[i][j].insert(2,Math.round((filtered[i][j][1]/frequency_of_phase[0])*100)+"%");
      }
    }
    for(i=1;i<filtered.length;i++){
      for(j=0;j<filtered[i].length;j++){
        var size=display_phase[i][j].length;
        //console.log(size);
        for(k=3;k<3+size;k++){
          filtered[i][j][k]=filtered[i][j][k]+" of "+ matchstr[i][j][k-3] +" in "+display_phase[i][j][k-3];
        }
      }
    }
    ...
    html+="<TH>Phase</TH><TH> Strata </TH><TH> Frequency(N) </TH><TH> Percentage(in terms of Phase)</TH>";
    ...
    //++++++++++++++++++++++++++++++ Statement For Strata Level ++++++++++++++++++++++++++++++++++

    for(i=0;i<strata_arr.length;i++){
    	html+="&nbsp;&nbsp;&nbsp;";
    	html+="<B>"+strata_arr[i][0]+"</B>" + " Strata has frequency "+"<B>"+strata_arr[i][1]+"</B>"+" ,which is "+"<B>"+strata_arr[i][2]+"</B>"+" of Phase " +"<B>"+phases_list[i]+"</B>"+"<br>";
   	}
    ...
    for(i=1;i<maxlength.length;i++){
    	html+="&nbsp;&nbsp;&nbsp;&nbsp;<B> TRANSITION PATTERNS OCCURING BETWEEN " + maxlength[i] + " PHASES</B>"+"<br><br>";
      	html+="<TABLE BORDER=ON style='position:relative;left:15px;'>";
      	html+="<TH>   Pattern of Transition  </TH><TH>  Frequency(N) </TH><TH> % of Total </TH>"; 
      	for(j=0;j<maxlength[i];j++){
        	html+="<TH>      % of "+stringifyNumber(j+1)+" phase strata   </TH>";
      	}
      	for(j=0; j<filtered[i].length; j++){
        	html+="<TR>";
        	for(k=0; k<filtered[i][j].length; k++){
            if(k==0){
              html+="<TD>"+filtered[i][j][k].join("-")+"</TD>";
            }
            else{
    ...
    html+="<br>";
    strata_arr_copy=strata_arr;
    console.log(strata_arr_copy);
    return html;
}
function GetStrata(){
  var check=[];
  ...
  var no_of_transitions=prompt("How many phase would you like to consider?"+"\n"+"Enter 1 to get proportion of strata"+"\n"+"Enter 2 or more to get proportion of Transitions across phases");
  for(i=0;i<parseInt(no_of_transitions);i++){
    var strataname=prompt("Enter Strata of Phase "+(i+1));
  ...
  function removeElementsWithValue(arr, val) {
    ...
    var found=strata_list.indexOf(check[0]);
    var pointer;
    if(check.length==1){
      for(i=0;i<strata_arr_copy.length;i++){
        if(strata_arr_copy[i][0]==check[0]){
          alert(check[0]+" has proportion of "+strata_arr_copy[i][2]+" in "+phases_list[found]);
        }
      }
    }
    else{
    ...
    for(i=0;i<filtered.length;i++){
      for(j=0;j<filtered[i].length;j++){
      withoutzero.push(removeElementsWithValue(filtered[i][j][0],"_"));
      }
    }
    //console.log(withoutzero);
    for(i=0;i<filtered.length;i++){
      for(j=0;j<filtered[i].length;j++){
        removeElementsWithValue(filtered[i][j][0],"_");
      }
    }
    //console.log(filtered);
    for(i=0;i<filtered.length;i++){
    ...
    //console.log(checked);
    var first = checked[1]+"("+checked[2]+")"+" transitions happens across \n";
    for(i=3;i<checked.length;i++){
      var second =" "+checked[i] + "\n";
      third+=second;
    }
    alert(first+third);
  }
